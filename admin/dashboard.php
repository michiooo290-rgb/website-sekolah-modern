...
$beritaPopuler  = db()->query('SELECT judul, kategori, tanggal, dilihat FROM berita ORDER BY dilihat DESC, tanggal DESC LIMIT 5')->fetchAll();
$pesanTerbaru   = db()->query('SELECT id, nama, subjek, tanggal, dibaca FROM pesan_kontak ORDER BY tanggal DESC LIMIT 5')->fetchAll();

$maxDilihat = !empty($beritaPopuler) ? max(1, (int)$beritaPopuler[0]['dilihat']) : 1;
...
<div class="admin-card bg-white/60 dark:bg-pine/40 backdrop-blur-sm rounded-2xl ring-1 ring-pine/8 dark:ring-cream/8 p-5 sm:p-6 mb-8">
  <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
    <?php
    $ringkas = [
      ['label' => 'Total Tayangan Berita', 'value' => number_format($dilihatTotal, 0, ',', '.'), 'sub' => 'akumulasi semua berita', 'd' => 'M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z'],
...
<!-- ============ Berita Terpopuler (lebar penuh) ============ -->
<div class="admin-card bg-white/60 dark:bg-pine/40 backdrop-blur-sm rounded-2xl ring-1 ring-pine/8 dark:ring-cream/8 p-6 mb-6">
  ...
  <?php else: ?>
    <div class="space-y-4">
      <?php foreach ($beritaPopuler as $i => $b):
        $persen = round(((int)$b['dilihat'] / $maxDilihat) * 100);
        if ($persen < 6) $persen = 6;
      ?>
        <div class="flex items-center gap-3 sm:gap-4">
          <span class="w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold shrink-0 <?php echo $i === 0 ? 'bg-brass text-pine-deep' : 'bg-pine/5 dark:bg-cream/10 text-pine/50 dark:text-cream/50'; ?>"><?php echo $i + 1; ?></span>
          <div class="flex-1 min-w-0">
            <div class="flex items-center justify-between gap-3 mb-1.5">
              <p class="text-sm font-semibold truncate">
                <?php echo esc($b['judul']); ?>
                <span class="text-[10px] text-pine/45 dark:text-cream/45 font-normal">· <?php echo esc($b['kategori']); ?></span>
              </p>
            ...
            <div class="h-1.5 rounded-full bg-pine/8 dark:bg-cream/10 overflow-hidden">
              <div class="h-full rounded-full <?php echo $i === 0 ? 'bg-brass' : 'bg-leaf/70'; ?>" style="width: <?php echo $persen; ?>%"></div>
            </div>
          </div>
        </div>
      <?php endforeach; ?>
