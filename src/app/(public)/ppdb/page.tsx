...
export default async function PpdbPage() {
  ...
  return <>
    ...
    <section className="relative z-10 bg-pine text-cream"><div className="max-w-6xl mx-auto px-5 py-20 sm:py-28 grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
      <div className="reveal">
        <p className="text-xs font-semibold tracking-widest text-brass-light uppercase mb-4">Lokasi</p>
        <h2 className="font-serif text-3xl sm:text-4xl leading-tight">Datang langsung ke sekolah.</h2>
        <p className="text-cream/70 mt-4 text-sm">{open ? "Pendaftaran dilakukan secara offline. Silakan kunjungi kami pada hari dan jam kerja." : "Pendaftaran sedang ditutup, namun sekolah tetap dapat dikunjungi pada hari dan jam kerja untuk bertanya."}</p>
        <ul className="space-y-4 text-cream/80 text-sm mt-8"><li><b className="text-cream">📍 Alamat</b><p className="text-cream/70">{settings.alamat}</p></li><li><b className="text-cream">📞 Telepon</b><p className="text-cream/70">{settings.telepon}</p></li><li><b className="text-cream">✉️ Email</b><p className="text-cream/70">{settings.email}</p></li></ul>
        <a href={petaTautan} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 mt-8 bg-brass text-pine-deep font-semibold px-6 py-3 rounded-full hover:bg-brass-light transition text-sm">
