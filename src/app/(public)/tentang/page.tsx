...
export default async function AboutPage() {
  ...
  return <>
    <OriginalBanner breadcrumb="Tentang" label="Mengenal Kami" title="Tentang SMAS Putra Persada" description="Mengenal lebih dekat sejarah, visi, dan dedikasi kami dalam membentuk generasi unggul." />
    <section className="relative z-10 max-w-6xl mx-auto px-5 py-20 sm:py-28"><div className="grid lg:grid-cols-2 gap-14 items-center"><div className="reveal"><div className="rounded-[2rem] overflow-hidden bg-gradient-to-br from-pine to-pine-deep aspect-[4/3] flex items-center justify-center shadow-xl"><Image src="/assets/img/logo.jpeg" alt="Logo SMAS Putra Persada" width={160} height={160} className="w-40 h-40 object-contain opacity-80" /></div></div><div className="reveal"><p className="text-xs font-semibold tracking-widest text-leaf dark:text-brass-light uppercase mb-4">Sejarah</p><h2 className="font-serif text-3xl sm:text-4xl text-pine dark:text-cream leading-tight mb-5">{history?.judul ?? "Dari mimpi sederhana menjadi sekolah terpercaya."}</h2><div className="text-pine/70 dark:text-cream/70 leading-relaxed space-y-4" dangerouslySetInnerHTML={{ __html: sanitizeHtml(history?.isi ?? "<p>Berdiri sejak tahun 2013, SMAS Putra Persada Batam terus bertumbuh menjadi sekolah yang mengintegrasikan keunggulan akademik dan karakter islami.</p>") }} /></div></div></section>
    ...
    <section className="relative z-10 max-w-6xl mx-auto px-5 py-20 sm:py-28"><div className="reveal text-center max-w-2xl mx-auto mb-14"><p className="text-xs font-semibold tracking-widest text-leaf dark:text-brass-light uppercase mb-4">Profil</p><h2 className="font-serif text-3xl sm:text-4xl text-pine dark:text-cream">Sekolah dalam angka.</h2></div><div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 reveal">{[["2013","Tahun Berdiri"],["750+","Peserta Didik"],["45+","Guru & Staf"],["25+","Ekstrakurikuler"]].map(([n,l]) => <div className="rounded-2xl border border-pine/10 dark:border-cream/10 p-7 text-center bg-cream-deep/40 dark:bg-pine/40" key={l}><p className="font-serif text-4xl text-brass dark:text-brass-light">{n}</p><p className="text-xs text-pine/60 dark:text-cream/60 mt-2 uppercase tracking-wide">{l}</p></div>)}</div></section>
    ...
    <OriginalCta title="Bergabunglah bersama kami." description="SMAS Putra Persada Batam — tempat terbaik untuk memulai perjalanan pendidikan putra-putri Anda." />
  </>;
}
